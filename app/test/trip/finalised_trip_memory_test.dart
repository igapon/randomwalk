...
void main() {
  ...
  group('pendingCelebration (M5 final review, Important I1)', () {
    ...
    test('returns null, with no marker to speak of, when none was set', () async {
      final memory = PrefsFinalisedTripMemory();
      expect(await memory.pendingCelebration(), isNull);
    });

    test(
      'a corrupt payload (not valid JSON) is treated as "nothing pending" '
      ...
      'without ever calling clearPendingCelebration',
      () async {
    ...
    test(
      'a payload that parses as JSON but has a corrupt stats sub-object is '
      'also treated as "nothing pending" AND removed',
      () async {
        final prefs = await SharedPreferences.getInstance();
  ...
  group('clearFinalisedTripMemoryPrefs (M5 final review, Important I1 / F1)', () {
    test('removes both finalised_trip_ids and pending_trip_celebration', () async {
      final memory = PrefsFinalisedTripMemory();
