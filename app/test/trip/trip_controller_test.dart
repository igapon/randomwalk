...
void main() {
  ...
  group('cold-start restore', () {
    ...
    test(
      'adopting an already-silent service shows the warning at once',
      () async {
    ...
    test(
      'a recording snapshot with no service means the process was killed',
      () async {
        tracker
          ..persisted = recordingSnapshot()
          ..running = false;
  ...
  group('starting a trip', () {
    ...
    test(
      'a planned route makes the trip route-bound and follows the camera',
      () async {
        bool? follow;
        final trip = build(onCameraFollowChanged: (v) => follow = v);
    ...
    test('double start is ignored, and says why', () async {
      ...
      expect(
        startFailureMessage(trip.lastStartFailure),
    ...
    test(
      'being sent to the settings screen does not start the service',
      () async {
        permissions = const TripPermissions(
  ...
  group('live progress', () {
    ...
    test(
      'elapsed counts from the trip start, not from the last update',
      () async {
        final trip = build();
        await trip.startTrip();
    ...
    test(
      'a tick samples the step sensor and publishes it to the service',
      () async {
        sensor.value = 1000;
        final trip = build();
        await trip.startTrip();
    ...
    test(
      'a tick never rewinds to a staler snapshot than the live one',
      () async {
        final trip = build();
        await trip.startTrip();
        tracker.emit(
    ...
    test(
      'granting "allow all the time" later clears the degraded mode',
      () async {
        permissions = const TripPermissions(
          outcome: TripPermissionOutcome.ready,
          mode: TrackingMode.foregroundOnly,
    ...
    test(
      'a silent GPS reaches a detached UI through the poll fallback',
      () async {
        final trip = build();
        await trip.startTrip();
        tracker.attached = false;

        tracker.persisted = recordingSnapshot().copyWith(
          gpsSilent: true,
          updatedAt: DateTime.utc(2026, 8, 30, 10, 30),
        );
        await trip.tick();
  ...
  group('stopping a trip', () {
    test(
      'banks the distance, submits the cumulative total, goes idle',
      () async {
        double? submitted;
    ...
    test('a snapshot resurrected after the stop is not banked twice', () async {
      totals.total = 10;
      final trip = build();
      await trip.startTrip();
      tracker.emit(recordingSnapshot(distanceKm: 2.4));
    ...
    test(
      'a genuinely different trip is still offered after a finalised one',
      () async {
        final trip = build();
        await trip.startTrip();
        tracker.emit(recordingSnapshot(distanceKm: 2.4));
    ...
    test('a double-tap on "Terminer" banks the trip exactly once (final '
        'review item 6)', () async {
      totals.total = 10;
      final trip = build();
      await trip.startTrip();
  ...
  group('speed history', () {
    ...
    test(
      'a bike trip records against the bike history, not the walk one',
      () async {
        final trip = build();
    ...
    test(
      'a session too short to be meaningful does not move the average',
      () async {
        ...
        expect(
          await realHistory.speedKmh(RoutingProfile.walk),
          closeTo(4.5, 1e-9),
        );
      },
    );
    ...
    test(
      'finishing an interrupted trip also records its session speed',
      () async {
        tracker
    ...
    test('a throwing SpeedHistoryStore does not stop the trip from finalising '
  ...
  group('game visits wiring (M4 Task 5)', () {
    test(
      'startTrip resolves poisFilePath and hands it to tracker.start',
      () async {
        final trip = build(
          resolvePoisFile: () async => '/tiles/v1/pois.json.gz',
        );
        await trip.startTrip();
    ...
    test(
      'a throwing resolvePoisFile never stops the trip from starting',
      () async {
        final trip = build(
          resolvePoisFile: () async => throw StateError('disk error'),
        );
        expect(await trip.startTrip(), isTrue);
        expect(tracker.startedPoisFilePath.single, isNull);
      },
    ...
    test(
      ...
      () async {
        ...
        tracker.emit(
          recordingSnapshot(distanceKm: 1.0).copyWith(pendingVisits: [visit]),
        );
    ...
    test(
      'a throwing processGameVisits never breaks snapshot adoption',
      () async {
        final trip = build(
        ...
        tracker.emit(
          recordingSnapshot(distanceKm: 3.0).copyWith(pendingVisits: [visit]),
        );
        await Future<void>.delayed(Duration.zero);
  ...
  group('Task 2g: auto-finish at arrival + congratulations screen', () {
    ...
    test('stopping a guided trip that HAS arrived fires the celebration '
      ...
      expect(
        await banked.pendingCelebration(),
        recordingSnapshot().startedAt.toUtc(),
      );
    });
    ...
    test('clearPendingCelebration/takePendingCelebration round-trip through '
      ...
      expect(
        await trip.takePendingCelebration(),
        recordingSnapshot().startedAt,
      );
  ...
  group('interrupted trip', () {
    ...
    test(
      '"Reprendre" does not inherit a silent-GPS warning from before',
      () async {
    ...
    test(
      '"Reprendre" keeps the original start time, so elapsed is continuous',
      () async {
        final trip = build();
        await trip.restore();
        await trip.resumeInterrupted();
        ...
        expect(
          tracker.startedWith.single.startedAt,
    ...
    test('"Reprendre" does not inherit a previous replan\'s route line '
    ...
    test('a double-tap on the interrupted banner\'s "Terminer" banks the '
    ...
    test('finishInterrupted racing stopTrip cannot double-bank either — the '
    ...
    test(
      '"Reprendre" re-checks permissions before restarting the service',
      () async {
        permissions = const TripPermissions(
          outcome: TripPermissionOutcome.locationDenied,
        );
  ...
  group('planned route state', () {
    ...
    test(
      'setting the profile without a planned route still remembers it',
      () async {
        final saved = <RoutingProfile>[];
        final trip = build(persistProfile: (p) async => saved.add(p));
        await trip.setProfile(RoutingProfile.bike);
