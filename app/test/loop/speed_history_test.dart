...
void main() {
  ...
  group('EMA convergence', () {
    test('one session moves the EMA by exactly alpha toward the sample',
        () async {
      final store = SpeedHistoryStore();
      // 6 km in 1h -> 6 km/h, a plausible walking speed.
      await store.recordSession(
          RoutingProfile.walk, 6.0, const Duration(hours: 1));

      // ema = 0.3*6 + 0.7*4.5 = 4.95
      expect(
          await store.speedKmh(RoutingProfile.walk), closeTo(4.95, 1e-9));
    });
  ...
  group('plausibility bounds', () {
    test('a walking session faster than 10 km/h is ignored', () async {
      ...
      await store.recordSession(
          RoutingProfile.walk, 12.0, const Duration(hours: 1));
      expect(await store.speedKmh(RoutingProfile.walk), closeTo(4.5, 1e-9));
    });
    ...
    test('a walking session slower than 2 km/h is ignored', () async {
      ...
      await store.recordSession(
          RoutingProfile.walk, 1.0, const Duration(hours: 1));
      expect(await store.speedKmh(RoutingProfile.walk), closeTo(4.5, 1e-9));
    });
    ...
    test('a bike session faster than 35 km/h is ignored', () async {
      ...
      await store.recordSession(
          RoutingProfile.bike, 40.0, const Duration(hours: 1));
      expect(await store.speedKmh(RoutingProfile.bike), closeTo(16.0, 1e-9));
    });
    ...
    test('a bike session slower than 8 km/h is ignored', () async {
      ...
      await store.recordSession(
          RoutingProfile.bike, 5.0, const Duration(hours: 1));
      expect(await store.speedKmh(RoutingProfile.bike), closeTo(16.0, 1e-9));
    });
    ...
    test('a plausible bike session updates the EMA', () async {
      ...
      await store.recordSession(
          RoutingProfile.bike, 20.0, const Duration(hours: 1));
  ...
  group('short-session ignore', () {
    test('a session of 300 m or less is ignored regardless of duration',
        () async {
      final store = SpeedHistoryStore();
      await store.recordSession(
          RoutingProfile.walk, 0.3, const Duration(hours: 1));
      expect(await store.speedKmh(RoutingProfile.walk), closeTo(4.5, 1e-9));
    });

    test('a session of 3 minutes or less is ignored regardless of distance',
        () async {
      final store = SpeedHistoryStore();
      await store.recordSession(
          RoutingProfile.walk, 5.0, const Duration(minutes: 3));
      expect(await store.speedKmh(RoutingProfile.walk), closeTo(4.5, 1e-9));
    });
    ...
    test('a session just over both thresholds is recorded', () async {
      ...
      await store.recordSession(RoutingProfile.walk, 0.31,
          const Duration(minutes: 3, seconds: 1));
      expect(await store.speedKmh(RoutingProfile.walk), isNot(closeTo(4.5, 1e-9)));
    });
  });

  group('persistence', () {
    test('the EMA survives a new store instance reading the same prefs',
        () async {
      final store = SpeedHistoryStore();
      await store.recordSession(
          RoutingProfile.walk, 6.0, const Duration(hours: 1));
      ...
      expect(
          await reopened.speedKmh(RoutingProfile.walk), closeTo(4.95, 1e-9));
    });

    test('it is stored under the documented shared_preferences keys',
        () async {
      final store = SpeedHistoryStore();
      await store.recordSession(
          RoutingProfile.walk, 6.0, const Duration(hours: 1));
