...
void main() {
  ...
  group('LoopPlanner — loop', () {
    ...
    test('same seed is deterministic across runs', () async {
      ...
      final fakeB = FakeRouter();
      final a =
          await LoopPlanner(router: fakeA.route).plan(loopRequest(seed: 99));
      final b =
          await LoopPlanner(router: fakeB.route).plan(loopRequest(seed: 99));
    ...
    test('bearings come from the injected rng, sub-seeded per candidate',
        () async {
      final log = CallLog();
      final fake = FakeRouter();
      await LoopPlanner(router: log.wrap(fake.route), rng: log.rng)
  ...
  group('LoopPlanner — repeated-segment weighting', () {
    ...
    final outAndBack = densify(
      [start, backtrack, start, backtrack, start],
      8,
    );
    ...
    test('a fully retracing loop scores 0.4 on the repeated term', () async {
      final result = await LoopPlanner(router: onTargetOutAndBack(5))
  ...
  group('LoopPlanner — short loops', () {
    test('repeated ratio does not saturate on a clean 300 m loop', () async {
      final fake = FakeRouter();
      final result =
          await LoopPlanner(router: fake.route).plan(loopRequest(targetKm: 0.3));
  ...
  group('LoopPlanner — deduplication', () {
    test('rotation-invariant candidates collapse to one offer', () async {
      ...
      final log = CallLog();
      final fake = FakeRouter();
      final result = await LoopPlanner(router: log.wrap(fake.route), rng: log.rng)
          .plan(loopRequest());

      expect(log.seeds, hasLength(LoopPlanner.candidateCount),
    ...
    test('genuinely different lengths are both kept', () async {
      ...
      final result =
          await LoopPlanner(router: scripted.route).plan(loopRequest(targetKm: 5));
  ...
  group('LoopPlanner — failure and budget', () {
    ...
    test('never exceeds 4 router calls for any single candidate', () async {
      ...
      expect(log.callsPerCandidate,
          hasLength(LoopPlanner.candidateCount));
      for (final calls in log.callsPerCandidate) {
        expect(calls, lessThanOrEqualTo(LoopPlanner.maxRouterCallsPerCandidate));
      }
      expect(log.callsPerCandidate, [4, 4, 4]);
    });

    test('a later, worse probe never displaces an earlier better one',
        () async {
