...
void main() {
  ...
  group('NavigationRuntime', () {
    ...
    test(
      'follows the seeded route and reports its shape from the first fix',
      () async {
    ...
    test(
      'lastUpdate exposes the raw follower update behind the fields',
      () async {
        final replan = FakeReplan([]);
        final nav = runtime(replan);
    ...
    test(
      'an off-route walker triggers exactly one replan onto a fresh route',
      () async {
        final replacement = straightRoute(
          lonOffset: eastDegrees(46.52, 60),
          turn: 'Chemin de Bellerive',
        );
    ...
    test(
      'a divergence big enough to look like a GPS spike still replans',
      () async {
        ...
        final replacement = straightRoute(
          lonOffset: eastDegrees(46.52, 250),
          turn: 'Route de Berne',
        );
        final replan = FakeReplan([replacement]);
        final nav = runtime(replan);
    ...
    test(
      'a replan that fails keeps the old route and is not retried for 30 s',
      () async {
    ...
    test(
      'a replan answering with an unusable route counts as a failure',
      () async {
        final replan = FakeReplan([
          const RouteResult(
            shape: [(46.52, 6.63)],
    ...
    test(
      'the pace estimate survives a replan, so the ETA never blanks out',
      () async {
        final replacement = straightRoute(lonOffset: eastDegrees(46.52, 60));
        final replan = FakeReplan([replacement]);
        final nav = runtime(replan);
    ...
    group('isLoop (final review item 1)', () {
      ...
      test(
        'an off-route loop is never replanned, but still reads off-route',
        () async {
          ...
          final replan = FakeReplan([
            straightRoute(lonOffset: eastDegrees(46.52, 60)),
          ]);
  ...
  group('navNotificationText', () {
    ...
    test('rounds to 10 m below 100 m and to 50 m above it', () {
      expect(
        navNotificationText(fields(distanceToManeuverM: 84)),
        startsWith('↰ Rue de Bourg · 80 m'),
      );
      expect(
        navNotificationText(fields(distanceToManeuverM: 96)),
        startsWith('↰ Rue de Bourg · 100 m'),
      );
      expect(
        navNotificationText(fields(distanceToManeuverM: 137)),
        startsWith('↰ Rue de Bourg · 150 m'),
      );
      expect(
        navNotificationText(fields(distanceToManeuverM: 1420)),
    ...
    test('an ETA the estimator will not commit to is simply left out', () {
      expect(
        navNotificationText(fields(etaSeconds: null)),
        '↰ Rue de Bourg · 100 m\n2,4 km',
      );
    });
    ...
    test('a route with no instruction still says what to do', () {
      expect(
        navNotificationText(fields(instruction: null)),
        startsWith("Suivez l'itinéraire · 100 m"),
      );
      expect(
        navNotificationText(fields(instruction: '')),
        startsWith("Suivez l'itinéraire · 100 m"),
      );
    });
