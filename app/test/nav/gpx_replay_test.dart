...
List<NavUpdate> _replay(String fixtureName) {
  final xml =
      File('test/nav/fixtures/$fixtureName.gpx').readAsStringSync();
...
void main() {
  ...
  group('nominal fixture', () {
    test('never off-route, arrives, maneuvers advance in order through all 4', () {
      final updates = _replay('nominal');

      expect(updates.any((u) => u.offRoute), isFalse,
          reason: 'a fix walked exactly on the route should never read as off-route');
      ...
      for (final u in updates) {
        expect(u.maneuverIndex, greaterThanOrEqualTo(prevIndex),
  ...
  group('jitter fixture', () {
    test('never off-route despite ±8 m noise, arrives, total progress ≈ route length', () {
      final updates = _replay('jitter');

      expect(updates.any((u) => u.offRoute), isFalse,
          reason: '±8 m gaussian noise should stay well clear of the 30 m '
  ...
  group('detour fixture', () {
    test('off-route triggers 10-20s after first exceeding 30m, clears on rejoin, arrives',
        () {
      final updates = _replay('detour');

      final firstExceedIndex =
          updates.indexWhere((u) => u.crossTrackM > 30);
      expect(firstExceedIndex, greaterThanOrEqualTo(0),
      ...
      final delay = points[firstOffRouteIndex]
          .time
          .difference(points[firstExceedIndex].time);
      ...
      final laterFalseIndex =
          updates.indexWhere((u) => !u.offRoute, firstOffRouteIndex + 1);
  ...
  group('tunnel fixture', () {
    test('no false arrival during the gap; resumes cleanly with a forward alongKm jump',
        () {
      final points =
          parseGpx(File('test/nav/fixtures/tunnel.gpx').readAsStringSync());
      ...
      expect(jumpM, greaterThan(20),
          reason: 'resuming after the gap should advance well beyond a '
