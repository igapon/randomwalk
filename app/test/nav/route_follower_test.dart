...
void main() {
  group('behavior 1: current maneuver and distanceToManeuverM', () {
    ...
    test('picks the first maneuver strictly ahead of alongKm', () {
      ...
      expect(
        u.distanceToManeuverM,
        closeTo(
          (geometry.cumulativeKm[4] - geometry.cumulativeKm[2]) * 1000,
          1,
        ),
      );
    });
    ...
    test(
      'last maneuver targets the end of the route, not its own position',
      () {
        final follower = RouteFollower(route);
        final fix = shape[6];
        final u = follower.update(fix.$1, fix.$2, DateTime(2026, 1, 1));
        ...
        expect(
          (u.distanceToManeuverM - toManeuverOwnPosition).abs(),
          greaterThan(50),
        );
      },
    );
  ...
  group('behavior 3: off-route timing uses update timestamps', () {
    test(
      'off-route only after >10s continuously beyond threshold, resets on a good fix',
      () {
        final base = (46.5200, 6.6300);
        final shape = <(double, double)>[
  ...
  group('behavior 4: arrival requires proximity AND last maneuver, and latches', () {
    test(
      'arrived stays false near destination until the last maneuver is active, then latches',
      () {
    ...
    group('arrival requires actual progress (final review item 5)', () {
      test(
        'a closed loop with no maneuvers does not arrive at its departure',
        () {
          ...
          expect(
            follower.update(farCorner.$1, farCorner.$2, t).arrived,
            isFalse,
          );
      ...
      test(
        'a degenerate route shorter than the arrival radius still arrives',
        () {
  ...
  group('ETA safety', () {
    test(
      'eta goes null after the estimator decays to a near-stationary speed',
      () {
        final shape = <(double, double)>[
    ...
    test(
      'eta is capped at 24h and never negative for a long remaining distance at low speed',
      () {
        final base = (46.5200, 6.6300);
  ...
  group(
    'behavior 5: a single aberrant fix is reported but does not move progress',
    () {
      test(
        'crossTrack > 200 m on one fix freezes alongKm/maneuverIndex, recovers after',
        () {
          final shape = <(double, double)>[
  ...
  group('RouteFollower ETA', () {
    test(
      'eta becomes available once the internal speed estimator has 3 samples',
      () {
        final shape = <(double, double)>[
          for (var i = 0; i <= 5; i++) (46.5200 + i * 0.001, 6.6300),
        ];
        ...
        final speedMps =
            (geometry.cumulativeKm[1] - geometry.cumulativeKm[0]) * 1000 / 10;
        final expectedEtaSeconds =
            (geometry.totalKm - geometry.cumulativeKm[3]) * 1000 / speedMps;
