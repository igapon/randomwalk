...
void main() {
  ...
  test('no journal file at all: resolves to the zero GameState, never throws',
      () async {
  ...
  test('a broken gameJournalProvider still resolves to the zero GameState',
      () async {
    final container = ProviderContainer(overrides: [
      gameJournalProvider.overrideWith((ref) async => throw Exception('boom')),
    ]);
    addTearDown(container.dispose);
  ...
  test('ref.invalidate(gameStateProvider) also forces a fresh replay',
      () async {
    final container = buildContainer();
    expect((await container.read(gameStateProvider.future)).coins, 0);
