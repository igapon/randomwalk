...
void main() {
  ...
  test(
      'parses features into GeocodeResult with correct lat/lon order '
      '(GeoJSON stores [lon, lat]) and a readable French label', () async {
    final client = MockClient((req) async => http.Response(
        jsonEncode(featureCollection([
          feature(lon: 6.6323, lat: 46.5197, properties: {
  ...
  test('skips a feature with no usable coordinates instead of throwing',
      () async {
    final client = MockClient((req) async => http.Response(
        jsonEncode(featureCollection([
  ...
  test('includes lat/lon bias query params only when nearLat/nearLon given',
      () async {
    Uri? withBias;
    Uri? withoutBias;
  ...
  test('non-200 HTTP response is surfaced as GeocodingException, not a crash',
      () async {
  ...
  test('network failure (offline) is surfaced as GeocodingException',
      () async {
    final client =
