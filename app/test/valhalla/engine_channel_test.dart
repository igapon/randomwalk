...
void main() {
  ...
  group('route()', () {
    test(
      'a null native reply raises RoutingException("empty engine reply")',
      () async {
  ...
  group('init()', () {
    test(
      'MissingPluginException on init is wrapped as RoutingException',
      () async {
        // No handler registered at all -> MissingPluginException.
        final engine = ChannelRoutingEngine();
        await expectLater(
    ...
    test(
      'a PlatformException on init is wrapped as RoutingException',
      () async {
        setHandler(
          (call) async =>
  ...
  group('quietenLoggingForRelease (item 7)', () {
    ...
    test('a release config silences every module\'s logging', () {
      ...
      for (final module in ['mjolnir', 'loki', 'thor', 'odin', 'meili']) {
        expect(
          (config[module] as Map)['logging']['type'],
          '',
    ...
    test(
      'a module with no logging block is left alone rather than crashing',
      () {
