...
|---|---|---|---|---|---|
| Position précise | **Oui** | Non | Fonctionnalité de l'app | Oui (rien n'est stocké côté serveur — voir note) | Traitée entièrement sur l'appareil (distance, map-matching, détection de lieux visités) ; **aucune coordonnée GPS n'est jamais transmise à un serveur**, quel qu'il soit. Déclarée "collectée" par prudence de revue (l'app lit activement le GPS pour sa fonction principale), pas "partagée". |
| Position approximative | **Oui** | Non | Fonctionnalité de l'app | Oui | Même traitement que ci-dessus. |
...
| Activité dans l'app (journal d'événements de jeu : XP, badges, visites, distance cumulée) | Oui, **seulement si un compte est créé** | Non | Fonctionnalité de l'app (synchronisation multi-appareils, classement) | Oui — "Supprimer mon compte" | Sans compte, rien de tout cela ne quitte l'appareil. |
